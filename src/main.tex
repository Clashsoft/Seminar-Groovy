...
In Zeile 3 wird die \code{duck}-Variable angelegt, der zunächst eine \code{Duck}-Instanz zugewiesen wird.
...
In der nächsten Zeile wird ein Objekt einer \code{Frog}-Klasse angelegt.
...
Hier werden zunächst zwei überladene \code{take}-Methoden definiert, die jeweils ein \code{Object} und einen \code{String} akzeptieren.
Die Variable in Zeile 3 hat den statischen Typ \code{Object}, ihr Wert ist zur Laufzeit aber vom Typ \code{String}.
Dadurch wählt die Groovy-Laufzeitumgebung bei dem Aufruf in Zeile 4 dynamisch die \code{take(String)}-Methode aus.
...
Für letztere sind der geschlossene \code{..}-Operator sowie der halboffene \code{..<}-Operator auch in anderen Kontexten verwendbar, beispielsweise in Foreach-Schleifen.
...
In Zeile 2 wird mithilfe der \code{collect}-Methode eine neue Liste erstellt, die jeweils das Doppelte der Elemente der Ursprungsliste enthält.
Dafür wird eine Closure als Argument übergeben, die einen Parameter \code{it} empfängt und ihn verdoppelt zurückgibt.
Danach werden mit \code{findAll} diejenigen Elemente in eine neue Liste gelegt, die durch zwei teilbar sind.
In Zeile 4 wird mit der \code{inject}-Methode die Summe der Elemente von \code{list} berechnet}.
...
In Zeile 5 wird die modifizierte Closure aufgerufen und der Wert der Ergebnisvariable ausgegeben.
Die Verwendung der dadurch definierten DSL, einer einfachen Rechenmaschine, folgt in Zeile 7.
...
Groovy macht diese einfacher als mit Java, indem es Erweiterungen für JDK-Klassen wie \code{java.io.File} bereitstellt.
Listing~\ref{lst:files.groovy} verwendet diese für eine beispielhafte Textverarbeitung.

\codelisting{groovy}{files.groovy}{Groovy-Skript zur Textverarbeitung}

Hier wird zunächst ein \code{File}-Objekt der Datei \code{all-files.txt} an eine Variable gebunden und durch Zuweisung des \code{text}-Attributs der Dateiinhalt geleert.
In Zeile 2 wird mit der \code{eachFileMatch}-Methode die übergebene Closure für jede Datei mit der Endung \code{.groovy} im aktuellen Verzeichnis ausgeführt.
Der Rumpf der Closure bewirkt zunächst in Zeile 3, dass der Name der aktuellen Datei \code{it} an \code{all-files.txt} angehangen wird\footnote{Dabei gehört \code{.name} zum Platzhalter in der Interpolation}.
Danach wird mit \code{eachLine} über die Zeilen von \code{it} iteriert, welche zusammen mit der Zeilennummer an \code{all-files.txt} angehangen werden.
Ergebnis des Skripts ist also die Datei \code{all-files.txt}, welche Groovy-Dateien mit Zeilennummern versehen auflistet.
...
In Zeile 3 wird schließlich die \code{factorial}-Methode aufgerufen.
...
Der Aufruf \code{0.factorial()} bezieht sich also auf die Definition aus Zeile 1.
Bei \code{5.factorial()} wird fünf Mal die Definition aus Zeile 2 aufgerufen, bevor \code{delegate} gleich \code{1} ist und \code{(delegate - 1).factorial()} auf die Definition für \code{0} verweist.
...
Zu letzteren zählen beispielsweise das Kopieren von Datein, Kompilieren von Modulen oder das Verpacken, Signieren und Veröffentlichen von Jar-Artefakten.
...
Es basiert auf der dem Backend-Framwork Spring Boot, setzt aber wie Rails auf Konvention statt Konfiguration, um einen leichteren Einstieg zu erlauben.
