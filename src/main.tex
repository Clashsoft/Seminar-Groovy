...
Die am meisten genutzte Programmiersprache der Welt ist Java.
...
Von Anfang an wollte der Entwickler

Im März 2004 wurde die neue Sprache dem Java Community Process unter Java Specification Request 241 eingereicht~\cite{jcp:jsr241}.
Nach einigen Vorgängerversionen und der Standardisierung wurde schließlich Version 1.0 wurde am 2. Januar 2007 veröffentlicht.
...
Ein wichtiger Aspekt, den Strachan in seinem Blogpost mit dem vereinfachten Test-Driven-Development andeutet, ist die Eignung für Domain-Specific Languages (DSLs).
Sie soll in den folgenden Texten besondere Aufmerksamkeit erhalten.
Neben Besonderheiten in Syntax und Semantik in Abschnitt~\ref{sec:sprache} werden auch das Ökosystem um Groovy sowie zwei der wichtigsten Framworks in Abschnitt~\ref{sec:community} betrachtet.
...
Auffällig ist das Schlüsselwort \code{def}, die Interpolierung in der Zeichenkette und der Methodenaufruf von \code{println} ohne Klammern.
...
\codelisting{groovy}{numbers.groovy}{Zahlen und mathematische Operatoren in Groovy}
...
Das Ergebnis von Divisionen ist hier auch ein \code{BigDecimal}, somit ist die Berechnung auch mit ``Brüchen'' korrekt.
...
Dieser kann implizit zu einem Bool konvertiert werden, der den Erfolg angibt, und ermöglich den Zugriff auf die Gruppen des regulären Ausdrucks.
...
Nach dem technischen Teil zur Spache soll nun auch das Ökosystem um Groovy betrachtet werden.

Im Januar 2019 ist Groovy auf Platz 21 des Tiobe Index~\cite{tiobe-index} und somit die zweithöchste JVM-Sprache (nach Java auf dem ersten Platz, vor Scala und Kotlin auf Platz 28. und 31.).
Auf StackOverflow~\cite{stackoverflow:groovy} sind über 22.000 Fragen und Antworten zu Groovy finden.
Die Popularität von Groovy ist wahrscheinlich mit dem einfachem Umstieg von Java und der Eignung als Skriptsprache zu erklären.

Da Groovy quelloffen~\cite{github:groovy} ist, kann sich jeder an der Weiterentwicklung und Fehlerbehebung beteiligen.
Im Folgenden werden kurz zwei Projekte vorgestellt, die auf Groovy basieren --- Gradle und Grails.
