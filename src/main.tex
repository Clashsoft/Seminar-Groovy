...
Design-Ziel der neuen Sprache war allgemein, Java-Entwicklern das Leben zu vereinfachen.
Durch kompakte Syntax sollte beispielweise Test-Driven Development vereinfacht werden.
...
Im Rumpf der Schleife wird die \code{quack}-Methode aufgerufen.
Bei den ersten zwei Iterationen enthält \code{o} eine \code{Duck}- bzw. eine \code{Frog}-Instanz, mit der die allgemeine \code{speak(Object)}-Methode aufgerufen wird.
Da das Argument zur Laufzeit eine Methode \code{quack} bereitstellt, kann diese (dynamisch) aufgerufen werden, obwohl \code{Object} diese nicht hat.
Die Ausgabe ist also zunächst \plain{I'm a duck} gefolgt von \plain{I'm a frog}.

In der dritten Iteration enthält \code{o} einen String.
Trotz des statischen Typs \code{Object}, wählt die Groovy-Laufzeitumgebung die \code{speak(String)}-Methode aus, da die Auflösung überladener Methoden auch anhand der dynamischen Argumenttypen erfolgt.
Die letzte Zeile der Ausgabe ist also \plain{A String can't talk}.
In Java würde die Methodenauswahl zur Kompilierungszeit erfolgen und die \code{speak(Object)}-Variante bevorzugen, da dort beim Überladen nur der statische Typ relevant ist.
...
Die Variablen können wie Funktionen aufgerufen werden, durch dynamische Typisierung auch mit Strings, wie in Zeile 2 zu sehen ist.
...
Mit Reduktion via \code{inject}, dem Startwert \code{0} und Reducer \code{adder} kann die Summe der Zahlen 1 bis 10 berechnet werden.
...
Somit kann das Programm erfolgreich ausgeführt werden und gibt \plain{5.5} aus.
...
Listing~\ref{lst:files.groovy} verwendet Erweiterungsmethoden von \code{java.io.File} für eine beispielhafte Textverarbeitungsaufgabe.
...
Der Rumpf der Closure bewirkt zunächst in Zeile 3, dass der Name der aktuellen Datei \code{it} an \code{all-files.txt} angehangen wird.
Danach wird mit \code{eachLine} über die Zeilen von \code{it} iteriert, welche zusammen mit ihrer Zeilennummer an \code{all-files.txt} angehangen werden.
...
Ein weiteres bekanntes Framework, Griffon, dient zum Erstellen von Desktop-Anwendungen basierend auf Swing und JavaFX, soll aber nicht näher erläutert werden.
Zudem existiert das Testing-Framework Spock, welches DSLs für Testfälle und Verhaltensspezifikation nutzt und auf JUnit aufbaut.
