...
Dennoch ist der Aufruf der \code{quack}-Methode möglich und erzeugt die Ausgabe \code{quack}.
...
Zeile 6 zeigt, dass \code{duck} noch immer eine Instanz der \code{Duck}-Klasse ist; der \code{quack}-Aufruf erzeugt jedoch nun die Ausgabe \code{I'm not a duck}.
...
Hier wurde zusätzlich ausgenutzt, dass die Verwendung des \code{return}-Schlüsselworts zur Rückgabe der erstellten Map auch optional ist.
...
Die Ausgabe ist zweimal der Satz \code{my cake contains sugar, eggs and flour}.
...
Jedoch wird sie von \code{eval} erst aufgerufen, nachdem das \code{delegate} gesetzt wurde.
Dies hat zur Folge, dass die bisher unbekannten Funktionen nun über das \code{delegate} zur Laufzeit aufgelöst werden, und sich entsprechend auf die Methoden der anonymen Klasse beziehen.
Somit kann das Programm erfolgreich ausgeführt werden und gibt \code{5.5} ($=(((0+3)*4)+-1)*0.5$) als Ergebnis der DSL-Rechnung aus.
